...
オードに書いてよいのは、**実際の会話に登場した内容だけ**です。会話で言及されていない地名、店名、アトラクション名、時間、予約、要素を、自分で創作したり推測で補ったりしてはいけません。会話に出ていない情報を書くことは、虚偽の記録を作ることと同義であり、絶対に避けてください。下のテンプレートに具体名（マゼランズ、ロストリバーデルタなど）が出てきても、それは「こういう形に整える」という構造の例にすぎず、コピーしてはいけません。

会話で言及されていない節は丸ごと省いてかまいません。例えばまだ Section が一つも語られていなければ、Sections の節は書かない。構成要素がまだ言語化されていなければ、構成要素の小節は書かない。未配属 Spot がなければ「未配属 Spots」の節も書かない。**空欄で埋めるより、節ごと削除する方が正しい。**
...
下のテンプレートに含まれる固有名詞は、すべてプレースホルダの一例です。実際の会話で出てきたものに置き換えるか、出てきていなければその節を削除してください。

先頭：

Ode <番号> — <旅のテーマを一行で。会話で出たキーワードを使う>

日付: <YYYY-MM-DD>
行き先: <会話で確定していれば書く。曖昧なら「未定」>

Story（コンセプトが言語化されていたら書く。していなければ節ごと省く）：

Story
  <Storyを一段落で。なぜこの旅なのか、何を旅するのか。会話で言われたことだけを書く>

  構成要素（Storyの構成要素が会話で引き出されていたら書く。していなければ小節ごと省く）
    <構成要素1の名前>: <会話で出た説明 / どのSpotで充当されているか>
    <構成要素2の名前>: ...

Sections（Spotが1つ以上会話に出ていたら書く。Sectionの名前は「<時間帯>の<エリア名>」の形を基本とする）：

Sections

  Section 1: <時間帯と場所>
    <会話に出たSpot名>
      <会話で確定したこと（時間、予約、補足）。Storyのどの要素を満たすかの一言>

  Section 2: <時間帯と場所>
    <会話に出たSpot名>
      <備考>

未配属 Spots（Sectionに入れにくいSpotが会話に出ていたら書く。なければ省く）：

未配属 Spots
  <会話に出たSpot名>
    <Section化が難しい理由、または再編待ち>

残された問い（未決事項が会話に出ていたら書く。なければ省く）：
...
事実関係（時間、固有名詞、予約状況）は会話に出たままを正確に書く。詩的に飾らない。

直近のやりとりから読み取れる新情報を、既存のオードの該当箇所に反映する。新しいSpotが挙がったら、会話で語られた時間帯やエリアをもとに、最も自然なSectionに入れる。どのSectionにも入らなければ「未配属 Spots」に置く。
